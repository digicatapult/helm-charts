...
| `cloudAgentPingTimeout`                     | Timeout in ms to wait for the next ping from the websocket server                                                | `30000`                                                          |
| `nyStateApiUrl`                             | Socrata API URL                                                                                                  | `https://data.ny.gov/resource/p66s-i79p.json`                    |
| `localRegistryToUse`                        | Registry that local instance's company number is registered with                                                 | `company_house`                                                  |
| `localRegistryCountryCode`                  | Country code for the local registry (iso country code)                                                           | `GB`                                                             |
| `openCorporatesApiUrl`                      | OpenCorporates API URL                                                                                           | `https://api.opencorporates.com/v0.4`                            |
| `openCorporatesApiKey.enabled`              | Enable openCorporates secret                                                                                     | `true`                                                           |
| `openCorporatesApiKey.secret`               | Secret value                                                                                                     | `openCorporatesApiKey`                                           |
| `openCorporatesApiKey.existingSecret`       | If there is an existing secret for the openCorporatesApiKey                                                      | `""`                                                             |
| `openCorporatesApiKey.existingSecretKey`    | the key to use within the existing secret                                                                        | `""`                                                             |
| `ipidApiUrl`                                | URL for iPiD service                                                                                             | `https://sandbox.ipid.works`                                     |
| `ipidCustomerId`                            | Customer ID provided by iPiD with API key                                                                        | `digicatapult-uat`                                               |
| `ipidApiKey.enabled`                        | Enable ipid secret                                                                                               | `true`                                                           |
| `ipidApiKey.secret`                         | Secret value                                                                                                     | `ipidApiKey`                                                     |
| `ipidApiKey.existingSecret`                 | If there is an existing secret for the ipidApiKey                                                                | `""`                                                             |
| `ipidApiKey.existingSecretKey`              | the key to use within the existing secret                                                                        | `""`                                                             |
...
| `smtpCredentials.existingSecret`            | SMTP existing secret name                                                                                        | `""`                                                             |
...
| `cnpg.nameOverride`                  | Unique name to prevent clobbering other CNPG clusters                                                                                                                                                                                                                                                                                                                                                                       | `veritable-ui-cnpg` |
...
| `cnpg.cluster.initdb.secret.name`    | Name of a pre-existing secret for the database owner (expects "username"/"password" keys within secret) to wire into the CNPG cluster in lieu of an auto-generated one. Leave unset to let CNPG auto-generate its own (default); set explicitly (matching cnpgSecrets.app.create's chart-managed secret, or your own externally-managed secret) to have CNPG adopt it instead                                               |                     |
...
| `cnpg.cluster.superuserSecret`       | Name of a pre-existing superuser secret (expects "username"/"password"/"pgpass" keys) to wire into the CNPG cluster in lieu of an auto-generated one. Leave unset to let CNPG auto-generate its own (default); set explicitly (matching cnpgSecrets.superuser.create's chart-managed secret, or your own externally-managed secret) to have CNPG adopt it instead                                                           |                     |
| `cnpgSecrets.app.create`             | Create a chart-managed CNPG app-user secret (named per cnpg.cluster.initdb.secret.name, or a cnpg.nameOverride-derived default); set cnpg.cluster.initdb.secret.name to have CNPG use it instead of auto-generating its own. Disabled by default so CNPG's own auto-generated secret (and its built-in connectivity test) are left untouched; enable together with cnpg.cluster.initdb.secret.name to pin known credentials | `false`             |
| `cnpgSecrets.superuser.create`       | Create a chart-managed CNPG superuser secret (named per cnpg.cluster.superuserSecret, or a cnpg.nameOverride-derived default); set cnpg.cluster.superuserSecret to have CNPG use it instead of auto-generating its own. Disabled by default so CNPG's own auto-generated secret (and its built-in connectivity test) are left untouched; enable together with cnpg.cluster.superuserSecret to pin known credentials         | `false`             |
...
| `cloudagent.cnpg.nameOverride` | Name for the veritable-cloudagent CNPG cluster dependency | `veritable-cloudagent-cnpg` |
