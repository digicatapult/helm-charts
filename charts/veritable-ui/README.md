...
| `image.tag`                                       | veritable-ui image tag (immutable tags are recommended)                                                                                                 | `v0.21.119`                 |
...
| `cnpg.cluster.superuserSecret`       | Name of a pre-existing superuser secret (expects "username"/"password"/"pgpass" keys) to wire into the CNPG cluster in lieu of an auto-generated one. Leave unset to let CNPG auto-generate its own                                                                                                                                                                           |                     |
| `cnpgSecrets.app.create`             | Create a chart-managed CNPG app-user secret (named per cnpg.cluster.initdb.secret.name, or a cnpg.nameOverride-derived default); set cnpg.cluster.initdb.secret.name to have CNPG use it instead of auto-generating its own                                                                                                                                                   | `false`             |
| `cnpgSecrets.superuser.create`       | Create a chart-managed CNPG superuser secret (named per cnpg.cluster.superuserSecret, or a cnpg.nameOverride-derived default); set cnpg.cluster.superuserSecret to have CNPG use it instead of auto-generating its own                                                                                                                                                        | `false`             |
